...
function RostersTab() {
  ...
  function toggleKeeperSelected(i: number, checked: boolean) {
    if (checked) {
      const selectedCount = entries.filter((e) => e.keeper_selected).length;
      if (selectedCount >= 2) {
        setMsg("Only 2 keepers can be selected — uncheck one first.");
        return;
      }
    }
    setMsg("");
    update(i, { keeper_selected: checked });
  }
  ...
  return (
    <div>
      ...
      <button onClick={addPlayer} className="mb-2 rounded-md bg-teal px-4 py-2 text-xs font-bold uppercase tracking-wide text-ink">
        + Add Player
      </button>
      <p className="mb-4 text-xs text-mute">
        Keeping: {entries.filter((e) => e.keeper_selected).length} / 2 selected
      </p>

      <div className="space-y-3">
        {entries.map((e, i) => (
          <div key={e.id} className="stat-card grid gap-2 rounded-xl p-4 sm:grid-cols-7">
            ...
            <label className="flex items-center gap-2 text-xs text-mute">
              <input type="checkbox" checked={e.keeper_eligible} onChange={(ev) => update(i, { keeper_eligible: ev.target.checked, keeper_selected: ev.target.checked ? e.keeper_selected : false })} />
              Eligible
            </label>
            ...
            <label className={`flex items-center gap-2 text-xs ${e.keeper_eligible ? "text-teal" : "text-mute/40"}`}>
              <input
                type="checkbox"
                checked={e.keeper_selected}
                disabled={!e.keeper_eligible}
                onChange={(ev) => toggleKeeperSelected(i, ev.target.checked)}
              />
              Keeping (max 2)
            </label>
            <label className="flex items-center gap-2 text-xs text-mute">
            ...
            <input className={`${inputCls} sm:col-span-6`} value={e.notes ?? ""} onChange={(ev) => update(i, { notes: ev.target.value })} placeholder="Notes (optional)" />
...
function FinancesTab() {
  ...
  const [msg, setMsg] = useState("");
  const [section, setSection] = useState<"dues" | "payouts" | "costs" | "weekly">("dues");
  ...
  async function load() {
    if (!seasonId) return;
    const [{ data: e }, { data: p }, { data: c }, { data: w }] = await Promise.all([
      ...
      supabase.from("weekly_results").select("*").eq("season_id", seasonId).order("week", { ascending: true }),
    ]);
    setEntries(e ?? []);
    setPayouts(p ?? []);
    setCosts(c ?? []);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-2">
          {(["dues", "payouts", "costs", "weekly"] as const).map((s) => (
            <button
              key={s}
      ...
      {section === "weekly" && <WeeklyResultsSection seasonId={seasonId} owners={owners} weeklyResults={weeklyResults} onChange={load} setMsg={setMsg} />}
    </div>
  );
}
...
function WeeklyResultsSection({
  ...
  const [local, setLocal] = useState<WeeklyResult[]>(weeklyResults);
  useEffect(() => { setLocal(weeklyResults); }, [weeklyResults]);

  async function add() {
    const nextWeek = local.length ? Math.max(...local.map((w) => w.week)) + 1 : 1;
    const { error } = await supabase.from("weekly_results").insert({ season_id: seasonId, week: nextWeek });
    setMsg(error ? `Error: ${error.message}` : "Added.");
    onChange();
  }
  async function save(w: WeeklyResult) {
    const { id, ...rest } = w;
    const { error } = await supabase.from("weekly_results").update(rest).eq("id", id);
    setMsg(error ? `Error: ${error.message}` : "Saved.");
  }
  async function remove(id: string) {
    await supabase.from("weekly_results").delete().eq("id", id);
    onChange();
  }

  return (
    <div>
      <p className="mb-3 text-xs text-mute">Weekly high/low score winner &amp; loser — drives the Weekly Winners list on the public Finances page.</p>
      <button onClick={add} className="mb-4 rounded-md bg-teal px-4 py-2 text-xs font-bold uppercase tracking-wide text-ink">
        + Add Week
      </button>
      <div className="space-y-2">
        {local.map((w, i) => (
          <div key={w.id} className="stat-card grid gap-2 rounded-xl p-3 sm:grid-cols-4">
            <input
              type="number"
              className={inputCls}
              value={w.week}
              onChange={(ev) => setLocal((prev) => prev.map((x, idx) => (idx === i ? { ...x, week: +ev.target.value } : x)))}
              placeholder="Week"
            />
            <select
              className={inputCls}
              value={w.winner_owner_id ?? ""}
              onChange={(ev) => setLocal((prev) => prev.map((x, idx) => (idx === i ? { ...x, winner_owner_id: ev.target.value || null } : x)))}
            >
            ...
            <select
              className={inputCls}
              value={w.loser_owner_id ?? ""}
              onChange={(ev) => setLocal((prev) => prev.map((x, idx) => (idx === i ? { ...x, loser_owner_id: ev.target.value || null } : x)))}
            >
            ...
            <div className="flex gap-2">
              <button onClick={() => save(local[i])} className="flex-1 rounded-md bg-teal px-3 py-2 text-xs font-bold uppercase tracking-wide text-ink">Save</button>
              <button onClick={() => remove(w.id)} className="rounded-md border border-line px-3 py-2 text-xs text-ember">✕</button>
            </div>
          </div>
        ))}
        {local.length === 0 && <p className="text-mute">No weeks entered for this season yet.</p>}
      </div>
    </div>
  );
}
