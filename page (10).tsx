...
import { supabase, Owner, Season, FinanceEntry, SeasonPayout, SeasonCost, WeeklyResult } from "@/lib/supabase";
  ...
  return (
    <div>
      ...
      {weeklyResults.length > 0 && (
        <div className="mt-12">
          <h2 className="font-display text-xl text-bone">Weekly Winners</h2>
          <div className="divider-tentacle my-3" />
          <div className="overflow-x-auto">
            <table className="w-full min-w-[400px] max-w-lg border-collapse text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs uppercase tracking-widest text-mute">
              ...
              <tbody>
                {weeklyResults.map((w) => (
                  <tr key={w.id} className="border-b border-line/50">
                    <td className="py-2 pr-3 font-display text-base text-bone">{w.week}</td>
                    <td className="px-2 text-gold">{w.winner_owner_id ? ownerMap.get(w.winner_owner_id)?.name : "—"}</td>
                    <td className="px-2 text-ember">{w.loser_owner_id ? ownerMap.get(w.loser_owner_id)?.name : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
